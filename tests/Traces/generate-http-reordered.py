...
http_request_part2 = "Host: example.com\r\nUser-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
http_request_part3 = "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36\r\n"
...
# Packet 1 (first part)
req_packet1 = IP(src=client_ip, dst=server_ip) / TCP(sport=client_port, dport=server_port, flags="A", seq=1001, ack=2001) / Raw(load=http_request_part1)
...
req_packet2 = IP(src=client_ip, dst=server_ip) / TCP(sport=client_port, dport=server_port, flags="A", seq=1001+len(http_request_part1), ack=2001) / Raw(load=http_request_part2)
req_packet2.time = base_time + 0.007  # This packet arrives second-to-last
req_packets.append(req_packet2)

# Packet 3 (third part)
req_packet3 = IP(src=client_ip, dst=server_ip) / TCP(sport=client_port, dport=server_port, flags="A", seq=1001+len(http_request_part1)+len(http_request_part2), ack=2001) / Raw(load=http_request_part3)
req_packet3.time = base_time + 0.006  # This packet arrives in the middle
req_packets.append(req_packet3)

# Packet 4 (fourth part)
req_packet4 = IP(src=client_ip, dst=server_ip) / TCP(sport=client_port, dport=server_port, flags="A", seq=1001+len(http_request_part1)+len(http_request_part2)+len(http_request_part3), ack=2001) / Raw(load=http_request_part4)
...
req_packet5 = IP(src=client_ip, dst=server_ip) / TCP(sport=client_port, dport=server_port, flags="PA", seq=1001+len(http_request_part1)+len(http_request_part2)+len(http_request_part3)+len(http_request_part4), ack=2001) / Raw(load=http_request_part5)
