...
# Client -> Server
c2s = IP(src=client_ip, dst=server_ip) / UDP(sport=client_port, dport=server_port) / Raw(load=payload)
c2s.time = base_time

# Server -> Client
s2c = IP(src=server_ip, dst=client_ip) / UDP(sport=server_port, dport=client_port) / Raw(load=payload)
